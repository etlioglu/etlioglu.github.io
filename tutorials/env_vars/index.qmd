...
The newly added path of `$HOME/.local/bin` is prepended to `$PATH`, this is advised over appending `$PATH`. With this, `$PATH` is read after the newly added path and therefore would have priority. This makes a difference whenever there is a conflict.
